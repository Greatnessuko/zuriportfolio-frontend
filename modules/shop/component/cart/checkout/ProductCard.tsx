...
export default function ProductCard({
  ...
  const get_rating = () => {
    ...
    let content = ratings.map((rating, index) => {
      if (rating > 0) {
        return <img className="h-5 w-5" src="/assets/icons/star-fill.svg" alt="" key={index} />;
      } else {
        return <img className="h-5 w-5" src="/assets/icons/star-outline.svg" alt="" key={index} />;
      }
    });
